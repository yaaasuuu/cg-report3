...
// 頂点配列の計算関数
void calcurateVtnr(GLfloat vtnr[][6], float h, float r_top, float r_bottom) {
  GLfloat radian;                  // 角度 θ
  float ratio = r_top / r_bottom;  // 上底 / 下底 r/R

  for (int i = 0; i <= NUM_DIV; i++) {
    radian = 2 * M_PI / NUM_DIV * i;
    // 頂点のz座標の計算(上底,下底)
    vtnr[2 * i][2] = vtnr[2 * i][5] = ratio * cos(radian);
    vtnr[2 * i + 1][2] = vtnr[2 * i + 1][5] = cos(radian);
    // 頂点のx座標の計算(上底,下底)
    vtnr[2 * i][0] = vtnr[2 * i][3] = ratio * sin(radian);
    vtnr[2 * i + 1][0] = vtnr[2 * i + 1][3] = sin(radian);
    // オブジェクトの高さの設定
    vtnr[2 * i][1] = h / 2;           // 上底の頂点のy座標を1に
    vtnr[2 * i + 1][1] = -1 * h / 2;  // 下底の頂点のy座標を-1に
    // 法線の計算
    // vtnr[2 * i][4] = vtnr[2 * i + 1][4] = 0;  // 法線のy方向成分を0に
    vtnr[2 * i][3] = vtnr[2 * i + 1][3] =
        h * sin(radian) / sqrtf(pow(r_bottom - r_top, 2) + pow(h, 2));
    vtnr[2 * i][4] = vtnr[2 * i + 1][4] =
        (r_top - r_bottom) / sqrtf(pow(r_bottom - r_top, 2) + pow(h, 2));
    vtnr[2 * i][5] = vtnr[2 * i + 1][5] =
        h * cos(radian) / sqrtf(pow(r_bottom - r_top, 2) + pow(h, 2));
  }
}
...
//
static unsigned int idx[NUM_DIV];   // 上面の頂点座標指定用
static unsigned int idx2[NUM_DIV];  // 下面の頂点座標指定用
void init(void) {
  for (int i = 0; i < NUM_DIV; i++) {
    idx[i] = i * 2;
    idx2[i] = i * 2 + 1;
  ...
  float r_top = 0.5;
  float r_bottom = 1;
  calcurateVtnr(vtnr, h, r_top, r_bottom);
...
void display(void) {
  ...
  // 円柱の側面の描画
  glDrawArrays(GL_TRIANGLE_STRIP, 0, NUM_VERT);

  // 円柱の上下底面の描画
  glDrawRangeElements(GL_TRIANGLE_FAN, 0, NUM_DIV - 1, NUM_DIV, GL_UNSIGNED_INT,
                      idx);
  glDrawRangeElements(GL_TRIANGLE_FAN, 0, NUM_DIV - 1, NUM_DIV, GL_UNSIGNED_INT,
                      idx2);
