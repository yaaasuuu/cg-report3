...
#define NUM_DIV 100                                // 分割数(水平方向)
#define NUM_DIV_STACK 100                          // 分割数(垂直方向)
#define NUM_VERT_SLICE (2 * (NUM_DIV + 1))         // 帯1つ毎の頂点数
#define NUM_VERT (NUM_VERT_SLICE * NUM_DIV_STACK)  // 全頂点数
...
// 頂点配列の計算用関数
void calcurateVtnr(GLfloat vtnr[][6]) {
  GLfloat radian_slice;  // 角度 θ
  GLfloat radian_stack;  // 角度 φ
  int index;

  for (int j = 0; j <= NUM_DIV_STACK * 2 - 1; j++) {
    if (j % 2 != 0) {
      radian_stack = M_PI / 2 - M_PI / NUM_DIV_STACK * ((j + 1) / 2);
    } else {
      radian_stack = M_PI / 2 - M_PI / NUM_DIV_STACK * (j / 2);
    }

    for (int i = 0; i <= NUM_DIV; i++) {
      radian_slice = 2 * M_PI / NUM_DIV * i;
      // 基準点からの数
      index = 2 * i;
      if (j % 2 == 0 && j != 0) {
        // 基準点
        ...
        // 帯の上部の頂点を、1つ上の帯の下部の頂点と同じにする(最初の列は例外)
        vtnr[index][0] = vtnr[index][3] = vtnr[index - (2 * NUM_DIV + 1)][0];
        vtnr[index][1] = vtnr[index][4] = vtnr[index - (2 * NUM_DIV + 1)][1];
        vtnr[index][2] = vtnr[index][5] = vtnr[index - (2 * NUM_DIV + 1)][2];
      } else {
        // 基準点
        index += (j - 1) * (NUM_DIV + 1) + 1;
        // 最初の列のみ
        if (j == 0) index = 2 * i;

        // 頂点のx座標の計算(上底,下底)
        vtnr[index][0] = vtnr[index][3] = sin(radian_slice) * cos(radian_stack);
        // 頂点のy座標の計算(上底,下底)
        vtnr[index][1] = vtnr[index][4] = sin(radian_stack);
        // 頂点のz座標の計算(上底,下底)
        vtnr[index][2] = vtnr[index][5] = cos(radian_slice) * cos(radian_stack);
      }
    }
  }
}
...
#endif
  // 頂点属性関係の設定
  glVertexAttribPointer(VERTLOC, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(GLfloat), 0);
  glEnableVertexAttribArray(VERTLOC);
  glVertexAttribPointer(NORMLOC, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(GLfloat),
...
static GLfloat ambCol[3] = {0.5, 0.5, 0.5};  // 環境光色(RGB別強さ)
...
void display(void) {
  ...
  nglLoadMatrix(nglGetTopMatrixStack(&ms), pvm);
  ...
  for (i = 0; i < 3; i++)
    lDirv[i] =
        v[i] * lDirRot[0] + v[4 + i] * lDirRot[1] + v[8 + i] * lDirRot[2];
  ...
  // 帯ごとに描写
  for (int i = 0; i < NUM_DIV_STACK; i++) {
    glDrawArrays(GL_TRIANGLE_STRIP, NUM_VERT_SLICE * i, NUM_VERT_SLICE);
  }
